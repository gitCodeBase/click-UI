...
export class VendorBookingComponent implements OnInit {
  ...
  calculateRate(): number {
    var discountedAmt = this.bookItemForm.controls['editedDiscount'].value
    if(discountedAmt != undefined && discountedAmt != ""){
      this.total = this.item.price.baseRate + this.item.price.tax - discountedAmt;
    } else{
      this.total = this.item.price.baseRate + this.item.price.tax - this.item.price.discount;
    }
    return this.total;
  }
